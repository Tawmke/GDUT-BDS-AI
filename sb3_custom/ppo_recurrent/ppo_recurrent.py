    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                self.logger.record('train/values_perepoch', th.mean(values[mask]).item())
                self.logger.record('train/values_pred_perepoch', np.float(th.mean(values_pred[mask])))
                self.logger.record('train/returns_perepoch', th.mean(rollout_data.returns[mask]).item())
        ...
        # added recordings 20221021
        self.logger.record('train/values', th.mean(values[mask]).item())
        self.logger.record('train/values_pred', np.float(th.mean(values_pred[mask])))
        self.logger.record('train/returns', th.mean(rollout_data.returns[mask]).item())
