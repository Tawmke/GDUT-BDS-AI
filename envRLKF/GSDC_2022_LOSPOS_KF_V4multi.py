...
satnumKF_df = pd.read_csv(f'{dir_path}/env/raw_satnum_gpsl1.csv')
traj_sum_df = pd.read_csv(f'{dir_path}/env/traj_summary_2.csv')
higwayID_traj = (traj_sum_df['Type']=='highway') & (traj_sum_df['AKF_wlsobs_Rcorr']=='improve') # & (traj_sum_df['AKF_wlsobs_Rcorr']=='improve')
...
class GPSPosition_continuous_lospos_convQR(gym.Env):
    ...
    def __init__(self,trajdata_range, traj_type, triptype, continuous_Xaction_scale, continuous_Vaction_scale,  continuous_actionspace, reward_setting, trajdata_sort, baseline_mod, traj_len, noise_scale_dic,
                 conv_corr):
    # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        ...
        self.continuous_Xaction_scale = continuous_Xaction_scale
        self.continuous_Vaction_scale = continuous_Vaction_scale
        self.action_space = spaces.Box(low=continuous_actionspace[0], high=continuous_actionspace[1], shape=(1, 6), dtype=np.float) # modified for RLKF
        self.total_reward = 0
    ...
    def step(self, action): # modified in 0411
        # judge if end #
        ...
        # action for new prediction, Q is vx, R is x
        action=np.reshape(action,[1,6])
        predict_vx = action[0,0]  #* self.continuous_Vaction_scale
        predict_vy = action[0,1]  #* self.continuous_Vaction_scale
        predict_vz = action[0,2]  #* self.continuous_Vaction_scale
        predict_x = action[0,3]  #* self.continuous_Xaction_scale
        predict_y = action[0,4]  #* self.continuous_Xaction_scale
        predict_z = action[0,5]  # * self.continuous_Xaction_scale
        ...
        # predict_Q_noise[0,:] *= action[0,0] * self.process_noise_scale
        # predict_Q_noise[1,:] *= action[0,1] * self.process_noise_scale
        # predict_Q_noise[2,:] *= action[0,2] * self.process_noise_scale
        # predict_R_noise[0,:] *= action[0,3] * self.measurement_noise_scale
        # predict_R_noise[1,:] *= action[0,4] * self.measurement_noise_scale
        # predict_R_noise[2,:] *= action[0,5] * self.measurement_noise_scale
        # 全输出
        ...
        # predict_R_noise = self.measurement_noise_scale * np.array([[action[0,9], action[0,10], action[0,11]],
        #                            [action[0,12], action[0,13], action[0,14]],
    ...
    def RL4KFGSDC(self,zs, us, predict_Q_noise,predict_R_noise): # RL for KF modified in 0303
        # Parameters
        sigma_mahalanobis = 10000000#30.0  # Mahalanobis distance for rejecting innovation
        dim_x = zs.shape[0]
        ...
        if self.conv_corr == 'conv_corr_1':
            Q = np.abs(self.covxv['covv'][int(self.current_step + (self.pos_num-1))] - predict_Q_noise) #
            ...
            Q = np.abs(predict_Q_noise)  #
        elif self.conv_corr == 'conv_corr_4':
            Q = np.abs(np.abs(self.covxv['covv'][int(self.current_step + (self.pos_num - 1))]) - np.abs(predict_Q_noise))  # 保证修正后协方差往小的方向走
        ...
        if d < sigma_mahalanobis:
            if self.conv_corr == 'conv_corr_1': # Estimated WLS position covariance
                R = np.abs(self.covxv['covx'][int(self.current_step + (self.pos_num-1))] - predict_R_noise)
            elif self.conv_corr == 'conv_corr_2':
                ...
                R =  np.abs(predict_R_noise)
            elif self.conv_corr == 'conv_corr_4':
                R = np.abs(np.abs(self.covxv['covx'][int(self.current_step + (self.pos_num - 1))]) - np.abs(predict_R_noise))  # 保证修正后协方差往小的方向走
...
class GPSPosition_continuous_lospos_convQRre(gym.Env):
    """
    correct the Q/R cov first and correct the initial KF position finally
    """
    metadata = {'render.modes': ['human']}
    def __init__(self,trajdata_range, traj_type, triptype, continuous_Xaction_scale, continuous_Vaction_scale, continuous_actionspace, reward_setting, trajdata_sort, baseline_mod, traj_len, noise_scale_dic,
                 conv_corr):
    # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(GPSPosition_continuous_lospos_convQRre, self).__init__()
        self.max_visible_sat=13
        self.pos_num = traj_len
        # self.observation_space = spaces.Box(low=-1, high=1, shape=(self.max_visible_sat, 4), dtype=np.float)#shape=(2, 1)
        self.observation_space = spaces.Dict({'gnss':spaces.Box(low=-1, high=1, shape=(1, self.max_visible_sat * CN0PRUEA_num)),
        ...
        self.continuous_Xaction_scale = continuous_Xaction_scale
        self.continuous_Vaction_scale = continuous_Vaction_scale
        self.action_space = spaces.Box(low=continuous_actionspace[0], high=continuous_actionspace[1], shape=(1, 12), dtype=np.float) # modified for RLKF
        self.total_reward = 0
        self.reward_setting=reward_setting
        self.trajdata_sort=trajdata_sort
        self.baseline_mod=baseline_mod
    ...
    def step(self, action): # modified in 0411
        ...
        predict_vy = action[0,1] * self.continuous_Vaction_scale
        predict_vz = action[0,2] * self.continuous_Vaction_scale
        Q_noise_vector = np.array([[action[0,3]], [action[0,4]], [action[0,5]]])
        predict_x = action[0,6] * self.continuous_Xaction_scale
        predict_y = action[0,7] * self.continuous_Xaction_scale
        predict_z = action[0,8] * self.continuous_Xaction_scale
        R_noise_vector = np.array([[action[0,9]], [action[0,10]], [action[0,11]]])
        predict_Q_noise = self.process_noise_scale * Q_noise_vector @ Q_noise_vector.T
        predict_R_noise = self.measurement_noise_scale * R_noise_vector @ R_noise_vector.T
        ...
        if self.baseline_mod == 'bl':
            ...
            obs_x = self.kf_realtime.loc[self.current_step + (self.pos_num-1), 'XEcefMeters_KF_realtime'] # WLS结果作为观测
            obs_y = self.kf_realtime.loc[self.current_step + (self.pos_num-1), 'YEcefMeters_KF_realtime']
            obs_z = self.kf_realtime.loc[self.current_step + (self.pos_num-1), 'ZEcefMeters_KF_realtime']
            kf_x = self.kf_realtime.loc[self.current_step + (self.pos_num-1), 'XEcefMeters_KF_realtime'] # 实时KF作为baselime
            kf_y = self.kf_realtime.loc[self.current_step + (self.pos_num - 1), 'YEcefMeters_KF_realtime']
            kf_z = self.kf_realtime.loc[self.current_step + (self.pos_num - 1), 'ZEcefMeters_KF_realtime']
            v_x = self.baseline_speed.loc[self.current_step + (self.pos_num - 1), 'VXEcefMeters_wls_igst']
        ...
        v_wls = np.array([v_x , v_y , v_z ])
        rl_x, rl_y, rl_z = self.RL4KFGSDC(x_wls,v_wls,predict_Q_noise,predict_R_noise)
        rl_x = rl_x + predict_vx + predict_x
        rl_y = rl_y + predict_vy + predict_y
        rl_z = rl_z + predict_vz + predict_z
...
class GPSPosition_continuous_lospos_convQRscale(gym.Env):
    """
    correct the Q/R cov by scale-based RL
    """
    metadata = {'render.modes': ['human']}
    def __init__(self,trajdata_range, traj_type, triptype, continuous_Xaction_scale, continuous_Vaction_scale, continuous_actionspace, reward_setting, trajdata_sort, baseline_mod, traj_len, noise_scale_dic,
                 conv_corr):
    # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(GPSPosition_continuous_lospos_convQRscale, self).__init__()
        self.max_visible_sat=13
        self.pos_num = traj_len
        # self.observation_space = spaces.Box(low=-1, high=1, shape=(self.max_visible_sat, 4), dtype=np.float)#shape=(2, 1)
        self.observation_space = spaces.Dict({'gnss':spaces.Box(low=-1, high=1, shape=(1, self.max_visible_sat * CN0PRUEA_num)),
        ...
        self.continuous_Xaction_scale = continuous_Xaction_scale
        self.continuous_Vaction_scale = continuous_Vaction_scale
        self.action_space = spaces.Box(low=continuous_actionspace[0], high=continuous_actionspace[1], shape=(1, 2), dtype=np.float) # modified for RLKF
        self.total_reward = 0
        self.reward_setting=reward_setting
        self.trajdata_sort=trajdata_sort
        self.baseline_mod=baseline_mod
    ...
    def step(self, action): # modified in 0411
        ...
        predict_Q_scale = np.abs(self.process_noise_scale * action[0,0])
        predict_R_scale = np.abs(self.measurement_noise_scale * action[0,1])
        ...
        x_wls = np.array([obs_x, obs_y , obs_z ])
        v_wls = np.array([v_x , v_y , v_z ])
        rl_x, rl_y, rl_z = self.RL4KFGSDC(x_wls,v_wls,predict_Q_scale,predict_R_scale)
    ...
    def RL4KFGSDC(self,zs, us, predict_Q_scale,predict_R_scale): # RL for KF modified in 0303
        # Parameters
        sigma_mahalanobis = 10000 # 30.0  # Mahalanobis distance for rejecting innovation
        dim_x = zs.shape[0]
        F = np.eye(dim_x)  # Transition matrix
        H = np.eye(dim_x)  # Measurement function
        # Initial state and covariance
        ...
        # KF: Prediction step
        ## Estimated WLS velocity covariance by predict scale
        Q = self.covxv['covv'][int(self.current_step + (self.pos_num-1))] * predict_Q_scale #
        self.covxv['covv'][int(self.current_step + (self.pos_num - 1))] = Q
        ...
        # KF: Update step
        # if d < sigma_mahalanobis:
        R = self.covxv['covx'][int(self.current_step + (self.pos_num-1))] * predict_R_scale
        self.covxv['covx'][int(self.current_step + (self.pos_num - 1))] = R
...
class GPSPosition_continuous_losposconvQR_onlyQR(gym.Env):
    metadata = {'render.modes': ['human']}
    ...
    # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(GPSPosition_continuous_losposconvQR_onlyQR, self).__init__()
        self.max_visible_sat=13
        self.pos_num = traj_len
        # self.observation_space = spaces.Box(low=-1, high=1, shape=(self.max_visible_sat, 4), dtype=np.float)#shape=(2, 1)
        self.observation_space = spaces.Dict({'gnss':spaces.Box(low=-1, high=1, shape=(1, self.max_visible_sat * CN0PRUEA_num)),
        ...
        self.continuous_Xaction_scale = continuous_Xaction_scale
        self.continuous_Vaction_scale = continuous_Vaction_scale
        self.action_space = spaces.Box(low=continuous_actionspace[0], high=continuous_actionspace[1], shape=(1, 18), dtype=np.float) # modified for RLKF
        self.total_reward = 0
        self.reward_setting=reward_setting
        self.trajdata_sort=trajdata_sort
        self.baseline_mod=baseline_mod
    ...
    def step(self, action): # modified in 0411
        ...
        x_wls = np.array([obs_x , obs_y , obs_z ])
        v_wls = np.array([v_x  , v_y , v_z  ])
        rl_x, rl_y, rl_z = self.RL4KFGSDC(x_wls,v_wls,predict_Q_noise,predict_R_noise, policy)
    ...
    def RL4KFGSDC(self,zs, us, predict_Q_noise,predict_R_noise,policy=True): # RL for KF modified in 0303
        # Parameters
        sigma_mahalanobis = 10000000000000 #30.0  # Mahalanobis distance for rejecting innovation
        dim_x = zs.shape[0]
        F = np.eye(dim_x)  # Transition matrix
        H = np.eye(dim_x)  # Measurement function
        # Initial state and covariance
        ...
        ## Estimated WLS velocity covariance
        if self.conv_corr == 'conv_corr_1':
            Q = self.covxv['covx'][int(self.current_step + (self.pos_num-1))]  + predict_R_noise #
        elif self.conv_corr == 'conv_corr_2':
            Q = self.covxv['covv'][int(self.current_step + (self.pos_num-2))] + predict_Q_noise #
...
class GPSPosition_continuous_lospos_convQR_QRNallcorrect(gym.Env):
    ...
    def _normalize_noise(self, obs_noise_R, obs_noise_Q):
        obs_noise_R = (obs_noise_R) / max(convx_max , np.abs(convx_min))
        obs_noise_Q = (obs_noise_Q) / max(convv_max , np.abs(convv_min))
        # obs_noise_Q = (obs_noise_Q - convv_mean) / convv_std
        # zero-score normalize
        # meanR, stdR = np.mean(obs_noise_R), np.std(obs_noise_R)
        # meanQ, stdQ = np.mean(obs_noise_Q), np.std(obs_noise_Q)
        # obs_noise_R = (obs_noise_R - meanR) * 0.1 / stdR
    ...
    def _next_observation(self):
        ...
        obs_Vnoise_cur = self.covxv['covv'][int(self.current_step + (self.pos_num - 1))]
        obs_noise_R = obs_Xnoise_pre  # np.concatenate((obs_Xnoise_pre, obs_Xnoise_cur))
        if self.conv_corr == 'conv_corr_2': # only for Q
            obs_noise_Q = obs_Vnoise_pre
        elif self.conv_corr == 'conv_corr_1':
            obs_noise_Q = obs_Vnoise_cur
        obs_noise_R, obs_noise_Q = self._normalize_noise(obs_noise_R, obs_noise_Q)
    ...
    def step(self, action): # modified in 3.3
        ...
        if self.allcorrect:
            ...
            x_wls = np.array([obs_x + predict_x,obs_y + predict_y,obs_z+ predict_z])
            v_wls = np.array([v_x + predict_Vx, v_y + predict_Vy, v_z + predict_Vz])
            rl_x, rl_y, rl_z = self.RL4KFGSDC(x_wls,v_wls,predict_R_noise,predict_Q_noise, policy)
            self.baseline.loc[self.current_step + (self.pos_num-1), ['X_RLpredict']] = rl_x
            self.baseline.loc[self.current_step + (self.pos_num-1), ['Y_RLpredict']] = rl_y
            self.baseline.loc[self.current_step + (self.pos_num-1), ['Z_RLpredict']] = rl_z
    ...
    def RL4KFGSDC(self,zs, us, predict_R_noise, predict_Q_noise, policy=True): # RL for KF modified in 0303
        # Parameters
...
class GPSPosition_continuous_lospos_convQR_onlyRNallcorrect(gym.Env):
    ...
    def step(self, action): # modified in 3.3
        ...
        predict_R_noise = self.measurement_noise_scale * np.array([[action[0,3], action[0,4], action[0,5]],
                                   [action[0,6], action[0,7], action[0,8]],
                                    [action[0,9], action[0,10], action[0,11]]])
        ...
        if self.allcorrect:
            ...
            x_wls = np.array([obs_x + predict_x* 0.5,obs_y + predict_y* 0.5,obs_z+ predict_z* 0.5])
            v_wls = np.array([v_x, v_y, v_z])
            rl_x, rl_y, rl_z = self.RL4KFGSDC(x_wls,v_wls,predict_R_noise)
            self.baseline.loc[self.current_step + (self.pos_num-1), ['X_RLpredict']] = rl_x
            self.baseline.loc[self.current_step + (self.pos_num-1), ['Y_RLpredict']] = rl_y
