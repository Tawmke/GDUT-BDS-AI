...
record_path = 'records_values/RL4KF_halftrain/'
afterRLAKF_path = 'source=urban_1_losposcovR_onlyRNallcorrect_conv_corr_2/' \
                  'continuous_lstmATF1_wls_igst_lr=0.0002_pos=10_QS=1e-10_RS=1e-05_allcorr=True_unit=64/RecurrentPPO_1/'
file_path = dir_path + record_path + afterRLAKF_path
# load raw data
with open(file_path+'raw_baseline_multic_afterRLAKF.pkl', "rb") as file:
    data_truth_dic_urban = pickle.load(file)
file.close()
...
with open(file_path + 'processed_features_multic_afterRLAKF.pkl', "rb") as file:
    losfeature_urban = pickle.load(file)
file.close()
"""
...
class GPSPosition_continuous_lospos(gym.Env):
    ...
    def reset(self):
        ...
        if self.triptype == 'urban':
            self.baseline=data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].copy()
            self.losfeature=losfeature_urban[self.tripIDlist[self.tripIDnum]].copy()
        self.kf_realtime = data_kf_realtime[self.tripIDlist[self.tripIDnum]].copy() # KF baseline
        ...
        # gen pred
        if self.baseline_mod == 'afterRLAKF':
            self.baseline['X_RLpredict'] = self.baseline['X_RLAKF']
            self.baseline['Y_RLpredict'] = self.baseline['Y_RLAKF']
            self.baseline['Z_RLpredict'] = self.baseline['Z_RLAKF']
        ...
        if self.traj_type[0] > 0:  # 只要剩下部分轨迹的定位结果
            if self.triptype == 'urban':
                data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[0:self.current_step - 1, ['X_RLpredict']] = None
                data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[0:self.current_step - 1, ['Y_RLpredict']] = None
                data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[0:self.current_step - 1, ['Z_RLpredict']] = None
    ...
    def _next_observation(self):
        ...
        if self.baseline_mod == 'afterRLAKF':
            obs = np.append(obs,[[self.baseline.loc[self.current_step + (self.pos_num-1), 'X_RLAKF']],
                                 [self.baseline.loc[self.current_step + (self.pos_num-1), 'Y_RLAKF']],
                                 [self.baseline.loc[self.current_step + (self.pos_num-1), 'Z_RLAKF']]],axis=1)
        elif self.baseline_mod == 'wls_igst':
        ...
        for i in range(len(self.visible_sat)):
            if self.visible_sat[i] in feature_tmp[:,0]:
                if feature_tmp[feature_tmp[:,0]==self.visible_sat[i],0] < 100: # 只需要GPSL1的卫星特征
                    feature_index_list = [1,2,3,4] # PRR and LOS
    ...
    def step(self, action):
        ...
        if self.baseline_mod == 'afterRLAKF':
            obs_x = self.baseline.loc[self.current_step + (self.pos_num-1), 'X_RLAKF']
            obs_y = self.baseline.loc[self.current_step + (self.pos_num-1), 'Y_RLAKF']
            obs_z = self.baseline.loc[self.current_step + (self.pos_num-1), 'Z_RLAKF']
            kf_x = self.baseline.loc[self.current_step + (self.pos_num-1), 'XEcefMeters_KF_realtime']
            kf_y = self.baseline.loc[self.current_step + (self.pos_num-1), 'YEcefMeters_KF_realtime']
            kf_z = self.baseline.loc[self.current_step + (self.pos_num-1), 'ZEcefMeters_KF_realtime'] # modified in 0412 17.45
        ...
        elif self.baseline_mod == 'kf_realtime': # 用实时卡尔曼滤波
        ...
        if self.triptype=='urban':
            data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[self.current_step + (self.pos_num-1), ['X_RLpredict']] = rl_x
            data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[self.current_step + (self.pos_num-1), ['Y_RLpredict']] = rl_y
            data_truth_dic_urban[self.tripIDlist[self.tripIDnum]].loc[self.current_step + (self.pos_num-1), ['Z_RLpredict']] = rl_z
        ...
        elif self.reward_setting=='RMSEadv_kfrealtime':
            reward = np.sqrt(((kf_x - gro_x) ** 2 + (kf_y - gro_y) ** 2 + (kf_z - gro_z) ** 2))*1e0 - \
                     np.sqrt(((rl_x - gro_x) ** 2 + (rl_y - gro_y) ** 2 + (rl_z - gro_z) ** 2))*1e0

        ecef_error = np.sqrt(((rl_x - gro_x) ** 2 + (rl_y - gro_y) ** 2 + (rl_z - gro_z) ** 2))
        if step_print:
        ...
        if done:
            ...
            obs = self._next_observation()
        return obs, reward, done, {'tripIDnum':self.tripIDnum, 'current_step':self.current_step, 'baseline':self.baseline,'error': ecef_error} #self.info#, {}# , 'data_truth_dic':data_truth_dic
